...
namespace BusinessLayeres // فضاء الأسماء اللي يحتوي الكود مال طبقة الـ Business.
{
    public static class clsGlobal // كلاس ثابت (static) ينخزن بيه متغيرات وإعدادات عامة.
    {
        ...
        public static decimal GetFinalOrderPrice(clsOrders order, decimal DeliveryPrice)
        {
        ...
        {
            // البحث عن الطلب الحالي حسب المعرف
            CurrentOrderInfo = clsOrders.FindOrder(orderID);

            // تحقق من أن الطلب موجود
            if (CurrentOrderInfo == null)
            {
                // إذا لم يتم العثور على الطلب، نوقف التنفيذ لتجنب NullReferenceException
                return;
            }

            // تحديث حالة الطلب إلى "ملغي" (4)
            CurrentOrderInfo.orderStatus = 4; // تغيير حالة الطلب

            // حفظ التاريخ كما هو (سطر عمليته هنا غير ضرورية، فقط نسخة)
            CurrentOrderInfo.orderDate = CurrentOrderInfo.orderDate;

            // حفظ الملاحظات كما هي (أيضًا مجرد إعادة تعيين)
            CurrentOrderInfo.Notes = CurrentOrderInfo.Notes;

            // تحديث المستخدم المسؤول عن الإلغاء، مع تحقق من null
            if (CurrentOrderInfo.UsersInfo != null && clsGlobal.CurrentUser != null)
            {
                CurrentOrderInfo.UsersInfo.UserName = clsGlobal.CurrentUser.UserName;
                CurrentOrderInfo.userID = clsGlobal.CurrentUser.UserID;
            }
            // ⚠️ ملاحظة: إذا UsersInfo أو CurrentUser كان null، الكود لن يكسر التطبيق

            // حفظ رقم الشخص المرتبط بالطلب
            CurrentOrderInfo.personID = CurrentOrderInfo.personID; // إعادة تعيين نفسه، مجرد نسخة

            // تحديث بيانات المنتج داخل الطلب، مع التحقق من null
            if (CurrentOrderInfo.OrderItemsInfo != null && CurrentOrderInfo.OrderItemsInfo.ProductInfo != null)
            {
                CurrentOrderInfo.OrderItemsInfo.ProductInfo.productID = CurrentOrderInfo.OrderItemsInfo.ProductInfo.productID;
                CurrentOrderInfo.OrderItemsInfo.ProductInfo.productName = CurrentOrderInfo.OrderItemsInfo.ProductInfo.productName;
            }
            // ⚠️ إذا أي من الخصائص null، سيتم تخطي التعيين لتجنب NullReferenceException

            // إعادة تعيين الوقت المقدر والوقت الفعلي لإنهاء الطلب (نسخ القيمة نفسها)
            CurrentOrderInfo.EstimateTime = CurrentOrderInfo.EstimateTime;
            CurrentOrderInfo.FinishTime = CurrentOrderInfo.FinishTime;

            // حفظ التغييرات في قاعدة البيانات
            if (!CurrentOrderInfo.Save())
            {
                // إذا فشل الحفظ، يمكن عرض رسالة خطأ
                MessageBox.Show("حدث خطأ أثناء حفظ الطلب.", "خطأ", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }

            // إنشاء حدث جديد للطلب لإرسال إشعار واتساب
            clsOrderNotifier.Order orderEventSource = new clsOrderNotifier.Order(CurrentOrderInfo.orderID);

            // إعداد خدمة الواتساب مع رسالة تلقائية للزبون
            clsOrderNotifier.WhatsappServiec whatsapp = new clsOrderNotifier.WhatsappServiec(
                "نعتذر عن إلغاء طلبكم ونتمنّى نخدمكم بأفضل شكل بالطلبات القادمة. \r\nمطبعة اتزان"
            );

            // ربط خدمة الواتساب بالطلب لإرسال الرسالة تلقائيًا عند إنشاء الحدث
            whatsapp.Subscribe(orderEventSource);

            // إنشاء الحدث مع حالة الطلب الملغي (4)
            orderEventSource.CreateOrder(4);
        }
