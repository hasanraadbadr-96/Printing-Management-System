    ...
    public class clsOrderNotifier // الكلاس الرئيسي لنظام اشعارات الطلبات
    {
        ...
        public class WhatsappServiec
        {
            ...
            public async Task ProcessOrderAndSendWhatsAppNotificationAsync(OrderEventArgs e)
            {
                ...
                await Task.Delay(2000); // ننتظر 4 ثواني  ونرسل الاشعار للواتساب async)
                clsOrderInvoiceExporter.SendInvoiceToWhatsapp( e.Phone, _MesseageTexet, clsGlobal.CurrentOrderInfo); // نرسل الفاتورة للعميل عبر الواتساب
