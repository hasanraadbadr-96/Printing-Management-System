...
using ETEZAN2024.Presenters.UserPresenters;
...
namespace ETEZAN2024.forms_And_Contrls.Users // مساحة الأسماء الخاصة بالمستخدمين
{
    public partial class frmUsersList : Form // كلاس يمثل نافذة عرض المستخدمين
    {
        ...
        clsUsers users = new clsUsers();
          clsUserPresenters userPresenters = new clsUserPresenters(); 
        ...
        private void _RefreshUsersList() // دالة تحديث كل المستخدمين
        {
            ...
            List<UsersDTO> usersList = userPresenters.LoadUsersList();

            // 2. ربط البيانات بـ DataGridView
            // 2. التعديل هنا: اربط القائمة اللي جلبناها مباشرة بالـ DataGridView
            dgvAllUsers.DataSource = usersList;
            // تعريب عناوين الأعمدة لتطابق الفلترة
            if (dgvAllUsers.Columns.Count > 0)
            {
                dgvAllUsers.Columns["UserID"].HeaderText = "معرف المستخدم";
                dgvAllUsers.Columns["UserName"].HeaderText = "اسم المستخدم";
                dgvAllUsers.Columns["Password"].HeaderText = "كلمة المرور";
                dgvAllUsers.Columns["PersonID"].HeaderText = "معرف الشخص";
                dgvAllUsers.Columns["UsersPermissionID"].HeaderText = "رقم الصلاحية";
                dgvAllUsers.Columns["IsActive"].HeaderText = "نشط";

                // لمسة احترافية: ترتيب العرض (اختياري)
                dgvAllUsers.Columns["UserID"].DisplayIndex = 0;
                dgvAllUsers.Columns["UserName"].DisplayIndex = 1;
            }
        ...
        private void _ApplyUserIFilter(string filterColumn, string filterValue)
        {
            // 1. جلب البيانات كـ List (بدل الـ DataTable)
            List<UsersDTO> fullList = userPresenters.LoadUsersList();

            if (fullList == null) return;

            // 2. إذا ماكو فلتر، نعرض القائمة كاملة ونخفي الأدوات
            if (string.IsNullOrEmpty(filterValue) && filterColumn == "لاشئ")
            {
                tbFilterValue.Visible = false;
                cmbIsActive.Visible = false;
                dgvAllUsers.DataSource = fullList;
                lbUsersCount.Text = fullList.Count.ToString();
                return;
            }

            // 3. تطبيق الفلترة باستخدام LINQ (هذا هو السحر البرمجي)
            IEnumerable<UsersDTO> filteredData = fullList;
            ...
            switch (filterColumn)
            {
                case "معرف المستخدم":
                    ...
                    if (!string.IsNullOrEmpty(filterValue))
                        filteredData = fullList.Where(u => u.UserID.ToString().Contains(filterValue));
                    break;
                ...
                case "معرف الشخص":
                    ...
                    cmbIsActive.Visible = false;
                    if (!string.IsNullOrEmpty(filterValue))
                        filteredData = fullList.Where(u => u.PersonID.ToString().StartsWith(filterValue));
                    break;
                ...
                case "اسم المستخدم":
                    ...
                    cmbIsActive.Visible = false;
                    if (!string.IsNullOrEmpty(filterValue))
                        filteredData = fullList.Where(u => u.UserName.ToLower().Contains(filterValue.ToLower()));
                    break;

                case "نشط":
                    tbFilterValue.Visible = false;
                    cmbIsActive.Visible = true;
                    if (cmbIsActive.Text == "نعم")
                        filteredData = fullList.Where(u => u.IsActive == true);
                    else if (cmbIsActive.Text == "لا")
                        filteredData = fullList.Where(u => u.IsActive == false);
                    break;
            }

            // 4. تحديث الجدول والعدد
            dgvAllUsers.DataSource = filteredData.ToList();
            lbUsersCount.Text = dgvAllUsers.RowCount.ToString();
        }
        ...
        private void ctmsDeleteUser_Click(object sender, EventArgs e) // حذف مستخدم
        {
            ...
            {
                if (users.DeleteUserById((int)dgvAllUsers.CurrentRow.Cells[0].Value)) // اذا نجح الحذف
                {
                    _RefreshUsersList();
                }
                else // اذا فشل الحذف
