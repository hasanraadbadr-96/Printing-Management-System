...
namespace ETEZAN2024.forms_And_Contrls.Orders // مساحة الاسم الخاصة بفورم الطلبات
{
    public partial class frmAddAndEditOrders : Form // تعريف كلاس الفورم الرئيسي للطلبات
    {
     ...
     //   private clsOrderItems _OrderItems; // كائن يمثل عناصر الطلب
        ...
        // دالة تتحقق اذا الشخص عنده دين نشط
        private bool _IsExistPersonActiveDebt( int PersonsID)
        {
           clsDebts debts = clsDebts.FindByPersonID(PersonsID);
             if (debts == null )
            {
                return false;
            }
            ...
            string message = string.Format(
                "هذا الشخص لديه دين سابق بقيمة [{0} د.ع].\n" +
                "لا يمكن إضافة طلب جديد له حتى يتم تسديد الدين كاملًا.",
                debts.debt_amount
            );
        ...
        private void btNext_Click(object sender, EventArgs e) // زر التالي
        {
            ...
            {// اذا اضافة طلب جديد
                ...
                // تحقق اذا الشخص المختار عنده دين نشط
                if (_IsExistPersonActiveDebt(ctrlPersonCardWithFilter1.PersonID))
                {
                    tpOrderInfo.Enabled = false;
                    return;
                }
        ...
        // زيادة عدد الطلبات للزبون
        private void _IncrementCustomerOrderCount(clsOrders order)
        {
            // هذا الفانكشن مسؤول عن زيادة عداد الطلبات للزبون
            // كل مرة يتم فيها تسجيل طلب مكتمل

            if (ctrlPersonCardWithFilter1.PersonID <= 0)
                return;
            // إذا ما كان هناك زبون مختار، نوقف العملية فوراً
            // هذا يمنع حدوث خطأ إذا PersonID غير صحيح أو صفر

            clsPersons person = clsPersons.FindPersonById(ctrlPersonCardWithFilter1.PersonID);
            // نجيب بيانات الزبون من قاعدة البيانات حسب PersonID
            // clsPersons.FindPersonById ترجع كائن الزبون إذا موجود، أو null إذا غير موجود

            if (person != null && order.orderStatus == 3)
            // نتأكد أن الزبون موجود فعلياً
            // ونتأكد أن حالة الطلب مكتملة (3 = Completed)
            {
                person.order_count += 1;
                // نزيد عداد الطلبات بمقدار واحد
                // هذا يضمن أن عدد الطلبات يعكس الواقع

                person.Save();
                // نحفظ التغيير في قاعدة البيانات
                // أي تحديث على عداد الطلبات يصبح فعلي ومباشر
            }
        }
              ...
              //  clsInventoryTransactionsNotifier.SaveToDataBase saveToData = new clsInventoryTransactionsNotifier.SaveToDataBase();
                ...
                if (_mode == enMode.Update)
                {
                   ...
                   // whatsapp1.Subscribe(InventoryTransaction);
                    ...
                    _IncrementCustomerOrderCount(Order); // نزود عدد الطلبات للزبون
